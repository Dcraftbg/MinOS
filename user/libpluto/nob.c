...
int main(int argc, char** argv) {
    ...
    if(!nob_mkdir_if_not_exists_silent(temp_sprintf("%s/libpluto", bindir))) return 1;
    ...
    for(size_t i = 0; i < dirs.count; ++i) {
        ...
        const char* dir = temp_sprintf("%s/libpluto/%s", bindir, dirs.items[i] + src_dir);
    ...
    for(size_t i = 0; i < c_sources.count; ++i) {
        ...
        const char* out = nob_temp_sprintf("%s/libpluto/%.*s.o", bindir, (int)(strlen(src + 4)-2), src + 4);
        da_append(&objs, out);
    ...
    const char* so = nob_temp_sprintf("%s/libpluto/libpluto.so", bindir);
    ...
    const char* archive = nob_temp_sprintf("%s/libpluto/libpluto.a", bindir);
